		...
		switch (state)
		{
			...
			case json_tokener_state_comment_eol:
				if (c == '\n')
				{
					if (mc_get_debug())
					{
						char *tmp = sstrndup(this->source + start_offset,
			...
			case json_tokener_state_string:
				if (c == quote_char)
				{
					printbuf_memappend(this->pb, this->source + start_offset, this->pos - start_offset);
			...
			case json_tokener_state_string_escape:
				switch (c)
				{
					case '"':
					case '\\':
						printbuf_memappend(this->pb, this->source + start_offset, this->pos - start_offset - 1);
						start_offset = this->pos++;
						state = saved_state;
						break;
					case 'b':
					...
					case 't':
						printbuf_memappend(this->pb, this->source + start_offset, this->pos - start_offset - 1);
						if (c == 'b')
							printbuf_memappend(this->pb, "\b", 1);
						else if (c == 'n')
							printbuf_memappend(this->pb, "\n", 1);
						else if (c == 'r')
							printbuf_memappend(this->pb, "\r", 1);
						else if (c == 't')
							printbuf_memappend(this->pb, "\t", 1);
						start_offset = ++this->pos;
						state = saved_state;
						break;
					case 'u':
				...
				if (strchr(json_hex_chars, c))
				{
					...
					if (this->pos - start_offset == 4)
					{
						...
						unsigned int ucs_char =
							...
							(hexdigit(*(this->source + start_offset + 2)) << 4) + hexdigit(*(this->source + start_offset + 3));
						if (ucs_char < 0x80)
						{
							utf_out[0] = ucs_char;
			...
			case json_tokener_state_boolean:
				if (strncasecmp("true", this->source + start_offset, this->pos - start_offset) == 0)
				{
					if (this->pos - start_offset == 4)
					{
				...
				else if (strncasecmp("false", this->source + start_offset, this->pos - start_offset) == 0)
				{
			...
			case json_tokener_state_object_field:
				...
				{
					printbuf_memappend(this->pb, this->source + start_offset, this->pos - start_offset);
					obj_field_name = sstrdup(this->pb->buf);
