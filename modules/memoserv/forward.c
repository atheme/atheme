...
static void ms_cmd_forward(sourceinfo_t *si, int parc, char *parv[])
{
	...
	if (si->su == NULL || !irccasecmp(si->su->nick, entity(si->smu)->name))
		myuser_notice(si->service->nick, tmu, "You have a new forwarded memo from %s (%d).", entity(si->smu)->name, LIST_LENGTH(&tmu->memos));
	else
		myuser_notice(si->service->nick, tmu, "You have a new forwarded memo from %s (nick: %s) (%d).", entity(si->smu)->name, si->su->nick, LIST_LENGTH(&tmu->memos));
	myuser_notice(si->service->nick, tmu, _("To read it, type /%s%s READ %d"),
				ircd->uses_rcommand ? "" : "msg ", si->service->disp, LIST_LENGTH(&tmu->memos));
