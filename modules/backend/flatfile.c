 ...
 *
 * $Id: flatfile.c 7843 2007-03-06 00:12:28Z pippijn $
 */
...
static void flatfile_db_save(void *arg)
{
	...
	{
		errno1 = errno;
		slog(LG_ERROR, _("db_save(): cannot create atheme.db.new: %s"), strerror(errno1));
	...
	slog(LG_DEBUG, _("db_save(): saving myusers"));

	dictionary_foreach(mulist, flatfile_db_save_myusers_cb, f);

	slog(LG_DEBUG, _("db_save(): saving mychans"));
	...
	/* Services ignores */
	slog(LG_DEBUG, _("db_save(): saving svsignores"));
	...
	/* Services operators */
	slog(LG_DEBUG, _("db_save(): saving sopers"));
	...
	slog(LG_DEBUG, _("db_save(): saving klines"));
	...
	{
		errno1 = errno;
		slog(LG_ERROR, _("db_save(): cannot write to atheme.db.new: %s"), strerror(errno1));
	...
	{
		errno1 = errno;
		slog(LG_ERROR, _("db_save(): cannot rename atheme.db.new to atheme.db: %s"), strerror(errno1));
...
static void flatfile_db_load(void)
{
	...
	if (f == NULL)
	{
		if (errno == ENOENT)
		{
			slog(LG_ERROR, _("db_load(): %s does not exist, creating it"), DATADIR "/atheme.db");
			return;
		}
		else
		{
			slog(LG_ERROR, _("db_load(): can't open %s for reading: %s"), DATADIR "/atheme.db", strerror(errno));
			slog(LG_ERROR, _("db_load(): exiting to avoid data loss"));
			exit(1);
		}
	}

	slog(LG_DEBUG, _("db_load(): ----------------------- loading ------------------------"));
	...
	while (fgets(dBuf, BUFSIZE, f))
	{
		...
		if (!strcmp("DBV", item))
		{
			...
			if (i > 5)
			{
				slog(LG_INFO, _("db_load(): database version is %d; i only understand 4 (Atheme 0.2), 3 (Atheme 0.2 without CA_ACLVIEW), 2 (Atheme 0.1) or 1 (Shrike)"), i);
				exit(EXIT_FAILURE);
			}
		}
		...
		if (!strcmp("CF", item))
		{
			s = strtok(NULL, " ");
			if (s == NULL)
				slog(LG_INFO, _("db_load(): missing param to CF"));
			else
			{
				their_ca_all = flags_to_bitmask(s, chanacs_flags, 0);
				if (their_ca_all & ~ca_all)
				{
					slog(LG_ERROR, _("db_load(): losing flags %s from file"), bitmask_to_flags(their_ca_all & ~ca_all, chanacs_flags));
				}
				if (~their_ca_all & ca_all)
				{
					slog(LG_ERROR, _("db_load(): making up flags %s not present in file"), bitmask_to_flags(~their_ca_all & ca_all, chanacs_flags));
				}
			}
		}
		...
		if (!strcmp("ME", item))
		{
			...
			if (!mu)
			{
				slog(LG_DEBUG, _("db_load(): memo for unknown account"));
				continue;
			}
		...
		if (!strcmp("MI", item))
		{
			...
			if (!mu)
			{
				slog(LG_DEBUG, _("db_load(): invalid ignore (MI %s %s)"),user,target);
				continue;
			}
		...
		if (!strcmp("AC", item))
		{
			...
			if (mu == NULL)
			{
				slog(LG_DEBUG, _("db_load(): invalid access entry<%s> for unknown user<%s>"), mask, user);
				continue;
			}
		...
		if (!strcmp("MN", item))
		{
			...
			{
				slog(LG_DEBUG, _("db_load(): invalid registered nick<%s> for user<%s>"), nick, user);
				continue;
			}
		...
		if (!strcmp("SO", item))
		{
			...
			if (!mu || !class || !flagstr)
			{
				slog(LG_DEBUG, _("db_load(): invalid services oper (SO %s %s %s)"), user, class, flagstr);
				continue;
			}
		...
		if (!strcmp("MC", item))
		{
			...
			if ((s = strtok(NULL, " ")))
			{
				...
				if (!mc->founder)
				{
					slog(LG_DEBUG, _("db_load(): channel %s has no founder, dropping."), mc->name);
					object_unref(mc);
					continue;
				}
		...
		if (!strcmp("CA", item))
		{
			...
			if (cachan && causer)
			{
				...
				{
					slog(LG_ERROR, _("db_load(): invalid chanacs (line %d)"),linecnt);
					continue;
				}
		...
		if (!strcmp("DE", item))
		{
			i = atoi(strtok(NULL, " "));
			if (i != muin)
				slog(LG_ERROR, _("db_load(): got %d myusers; expected %d"), muin, i);

			i = atoi(strtok(NULL, " "));
			if (i != mcin)
				slog(LG_ERROR, _("db_load(): got %d mychans; expected %d"), mcin, i);

			i = atoi(strtok(NULL, " "));
			if (i != cain)
				slog(LG_ERROR, _("db_load(): got %d chanacs; expected %d"), cain, i);

			if ((s = strtok(NULL, " ")))
				if ((i = atoi(s)) != kin)
					slog(LG_ERROR, _("db_load(): got %d klines; expected %d"), kin, i);
		}
	}

	fclose(f);

	slog(LG_DEBUG, _("db_load(): ------------------------- done -------------------------"));
}
