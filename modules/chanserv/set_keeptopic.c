...
cs_cmd_set_keeptopic(struct sourceinfo *si, int parc, char *parv[])
{
	...
	if (!strcasecmp("ON", parv[1]))
	{
		...
		verbose(mc, "\2%s\2 enabled the KEEPTOPIC flag", get_source_name(si));
	...
	else if (!strcasecmp("OFF", parv[1]))
	{
		...
		verbose(mc, "\2%s\2 disabled the KEEPTOPIC flag", get_source_name(si));
	...
	{
		command_fail(si, fault_badparams, STR_INVALID_PARAMS, "SET KEEPTOPIC");
		return;
	}
}
