 ...
 *
 * See doc/LICENSE for the licensing conditions of this.
 */
...
void MyMD5Init(struct MyMD5Context *context);
void MyMD5Update(struct MyMD5Context *context, unsigned char const *buf,
	       unsigned len);
void MyMD5Final(unsigned char digest[16], struct MyMD5Context *context);
void MyMD5Transform(uint32 buf[4], uint32 const in[16]);

/*
 * This is needed to make RSAREF happy on some MS-DOS compilers.
 */
typedef struct MyMD5Context MyMD5_CTX;
 ...
 */
void
MyMD5Init(struct MyMD5Context *ctx)
{
 ...
 */
void
MyMD5Update(struct MyMD5Context *ctx, unsigned char const *buf, unsigned len)
{
    uint32 t;
	...
	MyMD5Transform(ctx->buf, (uint32 *) ctx->in);
	buf += t;
	len -= t;
    }
 ...
 */
void
MyMD5Final(unsigned char digest[16], struct MyMD5Context *ctx)
{
    unsigned count;
    unsigned char *p;
 ...
 */
void
MyMD5Transform(uint32 buf[4], uint32 const in[16])
{
...
static const char *sorservices_crypt_string(const char *key, const char *salt)
{
	...
	base64_encode(md5_password(key), 16, buf, BUFSIZE);
	snprintf(outbuf, BUFSIZE, "$%s", buf);
