 ...
 *
 * Raw SHA2-256 password encryption.
 * Hash functions are not designed to encrypt passwords directly.
 */

#include <atheme.h>

#define MODULE_PREFIX_STR       "$rawsha256$"
#define MODULE_PREFIX_LEN       11
#define MODULE_DIGEST_LEN       DIGEST_MDLEN_SHA2_256
#define MODULE_PARAMS_LEN       (MODULE_PREFIX_LEN + (2 * MODULE_DIGEST_LEN))

static bool ATHEME_FATTR_WUR
atheme_rawsha256_verify(const char *const restrict password, const char *const restrict parameters,
                        unsigned int *const restrict flags)
{
	...
	*flags |= PWVERIFY_FLAG_MYMODULE;
	...
	if (! digest_oneshot(DIGALG_SHA2_256, password, strlen(password), digest, NULL))
		return false;
...
static const struct crypt_impl crypto_rawsha256_impl = {

	.id         = "rawsha256",
	.verify     = &atheme_rawsha256_verify,
};

static void
mod_init(struct module *const restrict m)
{
	(void) crypt_register(&crypto_rawsha256_impl);
...
{
	(void) crypt_unregister(&crypto_rawsha256_impl);
}

SIMPLE_DECLARE_MODULE_V1("crypto/rawsha256", MODULE_UNLOAD_CAPABILITY_OK)
