                             ...
                             const char __attribute__((unused)) *const restrict parameters)
{
	...
	if (snprintf(parv, PASSLEN, CRYPT3_SAVESALT_FORMAT_SHA2_256, salt) >= PASSLEN)
	{
                              ...
                              unsigned int *const restrict flags)
{
	...
	{
		(void) slog(LG_DEBUG, "%s: digest not %u characters long", __func__, CRYPT3_LOADHASH_LENGTH_SHA2_256);
		return false;
	}
