 ...
 *
 * $Id: sorcery.c 8203 2007-04-29 16:05:50Z jilles $
 */
...
DECLARE_MODULE_V1("protocol/sorcery", TRUE, _modinit, NULL, "$Id: sorcery.c 8203 2007-04-29 16:05:50Z jilles $", "Atheme Development Group <http://www.atheme.org>");
...
struct cmode_ sorcery_user_mode_list[] = {
  { 'i', UF_INVIS    },
  { 'o', UF_IRCOP    },
  { '\0', 0 }
};
...
static void sorcery_quit_sts(user_t *u, const char *reason)
{
	if (!me.connected)
		return;
...
/* PRIVMSG wrapper */
static void sorcery_msg(const char *from, const char *target, const char *fmt, ...)
{
	va_list ap;
	char buf[BUFSIZE];
...
/* PART wrapper */
static void sorcery_part_sts(channel_t *c, user_t *u)
{
...
/* topic wrapper */
static void sorcery_topic_sts(channel_t *c, const char *setter, time_t ts, time_t prevts, const char *topic)
{
	if (!me.connected)
		return;
...
/* mode wrapper */
static void sorcery_mode_sts(char *sender, channel_t *target, char *modes)
{
	if (!me.connected)
		return;
...
static void sorcery_jupe(const char *server, const char *reason)
{
	if (!me.connected)
		return;
...
void _modinit(module_t * m)
{
	...
	user_mode_list = sorcery_user_mode_list;
