...
static void hyperion_kick(user_t *source, channel_t *c, user_t *u, const char *reason)
{
...
static void hyperion_kline_sts(const char *server, const char *user, const char *host, long duration, const char *reason)
{
	if (!me.connected)
		return;
...
static void hyperion_unkline_sts(const char *server, const char *user, const char *host)
{
	if (!me.connected)
		return;
...
/* topic wrapper */
static void hyperion_topic_sts(channel_t *c, user_t *source, const char *setter, time_t ts, time_t prevts, const char *topic)
{
	if (!me.connected || !c)
		return;

	/* Send 0 channelts so this will always be accepted */
	sts(":%s STOPIC %s %s %lu 0 :%s", source->nick, c->name, setter,
