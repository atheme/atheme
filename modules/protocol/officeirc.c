...
DECLARE_MODULE_V1("protocol/officeirc", true, _modinit, NULL, PACKAGE_STRING, "Atheme Development Group <http://www.atheme.org>");
...
/* topic wrapper */
static void officeirc_topic_sts(channel_t *c, user_t *source, const char *setter, time_t ts, time_t prevts, const char *topic)
{
	if (!me.connected)
		return;
...
static void officeirc_on_login(user_t *u, myuser_t *account, const char *wantedhost)
{
...
/* protocol-specific stuff to do on login */
static bool officeirc_on_logout(user_t *u, const char *account)
{
	if (!me.connected || u == NULL)
		return false;
...
void _modinit(module_t * m)
{
	...
	ignore_mode_list_size = ARRAY_SIZE(officeirc_ignore_mode_list);
