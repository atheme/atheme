...
{
	slog(LG_DEBUG, "charybdis_chan_lowerts(): lowering TS for %s to %lu",
...
static void charybdis_kick(user_t *source, channel_t *c, user_t *u, const char *reason)
{
...
/* numeric wrapper */
static void charybdis_numeric_sts(server_t *from, int numeric, user_t *target, const char *fmt, ...)
{
	va_list ap;
	char buf[BUFSIZE];
