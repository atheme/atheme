...
static void ts6_introduce_nick(user_t *u)
{
	...
	if (ircd->uses_uid && use_euid)
		sts(":%s EUID %s 1 %lu %sS %.10s %s 0 %s * * :%s", me.numeric, u->nick, (unsigned long)u->ts, umode, u->user, u->host, u->uid, u->gecos);
	else if (ircd->uses_uid)
		sts(":%s UID %s 1 %lu %sS %.10s %s 0 %s :%s", me.numeric, u->nick, (unsigned long)u->ts, umode, u->user, u->host, u->uid, u->gecos);
	else
		sts("NICK %s 1 %lu %sS %.10s %s %s :%s", u->nick, (unsigned long)u->ts, umode, u->user, u->host, me.name, u->gecos);
}
