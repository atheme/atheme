 ...
 *
 * $Id: ratbox.c 8203 2007-04-29 16:05:50Z jilles $
 */
...
DECLARE_MODULE_V1("protocol/ratbox", TRUE, _modinit, NULL, "$Id: ratbox.c 8203 2007-04-29 16:05:50Z jilles $", "Atheme Development Group <http://www.atheme.org>");
...
static void ratbox_quit_sts(user_t *u, const char *reason)
{
	if (!me.connected)
		return;
...
/* WALLOPS wrapper */
static void ratbox_wallops_sts(const char *text)
{
...
/* PRIVMSG wrapper */
static void ratbox_msg(const char *from, const char *target, const char *fmt, ...)
{
	va_list ap;
	char buf[BUFSIZE];
...
/* NOTICE wrapper */
static void ratbox_notice_user_sts(user_t *from, user_t *target, const char *text)
{
	sts(":%s NOTICE %s :%s", from ? CLIENT_NAME(from) : ME, CLIENT_NAME(target), text);
}

static void ratbox_notice_global_sts(user_t *from, const char *mask, const char *text)
{
	node_t *n;
	tld_t *tld;
...
static void ratbox_notice_channel_sts(user_t *from, channel_t *target, const char *text)
{
...
static void ratbox_wallchops(user_t *sender, channel_t *channel, const char *message)
{
...
/* PART wrapper */
static void ratbox_part_sts(channel_t *c, user_t *u)
{
...
/* topic wrapper */
static void ratbox_topic_sts(channel_t *c, const char *setter, time_t ts, time_t prevts, const char *topic)
{
	int joined = 0;
...
/* mode wrapper */
static void ratbox_mode_sts(char *sender, channel_t *target, char *modes)
{
...
static void ratbox_jupe(const char *server, const char *reason)
{
	if (!me.connected)
		return;
...
static void m_encap(sourceinfo_t *si, int parc, char *parv[])
{
	...
	if (!irccasecmp(parv[1], "LOGIN"))
	{
		...
		{
			slog(LG_DEBUG, "m_encap(): %s sent ENCAP LOGIN for user %s outside burst", u->server->name, si->su->nick);
			return;
		}
...
void _modinit(module_t * m)
{
	...
	kick = &ratbox_kick;
	msg = &ratbox_msg;
	notice_user_sts = &ratbox_notice_user_sts;
	notice_global_sts = &ratbox_notice_global_sts;
