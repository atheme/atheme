...
DECLARE_MODULE_V1("protocol/snircd", true, _modinit, NULL, PACKAGE_STRING, "Atheme Development Group <http://www.atheme.org>");

/* *INDENT-OFF* */

ircd_t Snircd = {
        "Snircd 1.2.1 and later",        /* IRCd name */
...
struct cmode_ snircd_mode_list[] = {
  { 'i', CMODE_INVITE },
  { 'm', CMODE_MOD    },
  { 'n', CMODE_NOEXT  },
  { 'p', CMODE_PRIV   },
...
struct extmode snircd_ignore_mode_list[] = {
  { '\0', 0 }
};

struct cmode_ snircd_status_mode_list[] = {
  { 'o', CSTATUS_OP    },
  { 'v', CSTATUS_VOICE },
  { '\0', 0 }
};

struct cmode_ snircd_prefix_mode_list[] = {
  { '@', CSTATUS_OP    },
  { '+', CSTATUS_VOICE },
  { '\0', 0 }
};

struct cmode_ snircd_user_mode_list[] = {
  { 'i', UF_INVIS    },
  { 'o', UF_IRCOP    },
  { 'd', UF_DEAF     },
  { 'k', UF_IMMUNE   },
...
/* NOTICE wrapper */
static void snircd_notice_channel_sts(user_t *from, channel_t *target, const char *text)
{
...
static void snircd_wallchops(user_t *sender, channel_t *channel, const char *message)
{
...
static void snircd_on_login(user_t *u, myuser_t *account, const char *wantedhost)
{
...
static bool snircd_on_logout(user_t *u, const char *account)
{
...
void _modinit(module_t * m)
{
	...
	notice_channel_sts = &snircd_notice_channel_sts;
	wallchops = &snircd_wallchops;
	ircd_on_login = &snircd_on_login;
	ircd_on_logout = &snircd_on_logout;

	mode_list = snircd_mode_list;
	ignore_mode_list = snircd_ignore_mode_list;
	status_mode_list = snircd_status_mode_list;
	prefix_mode_list = snircd_prefix_mode_list;
	user_mode_list = snircd_user_mode_list;
	ignore_mode_list_size = ARRAY_SIZE(snircd_ignore_mode_list);
