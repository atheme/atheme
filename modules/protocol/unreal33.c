...
DECLARE_MODULE_V1("protocol/unreal33", true, _modinit, NULL, PACKAGE_STRING, "Atheme Development Group <http://www.atheme.org>");
...
ircd_t Unreal = {
        "UnrealIRCd 3.3 or later",      /* IRCd name */
...
struct cmode_ unreal33_mode_list[] = {
  { 'i', CMODE_INVITE   },
  { 'm', CMODE_MOD      },
  { 'n', CMODE_NOEXT    },
  { 'p', CMODE_PRIV     },
...
struct extmode unreal33_ignore_mode_list[] = {
...
struct cmode_ unreal33_status_mode_list[] = {
  { 'q', CSTATUS_OWNER   },
  { 'a', CSTATUS_PROTECT },
  { 'o', CSTATUS_OP      },
  { 'h', CSTATUS_HALFOP  },
...
struct cmode_ unreal33_prefix_mode_list[] = {
  { '*', CSTATUS_OWNER   },
  { '~', CSTATUS_PROTECT },
  { '@', CSTATUS_OP      },
  { '%', CSTATUS_HALFOP  },
...
struct cmode_ unreal33_user_mode_list[] = {
  { 'A', UF_ADMIN    },
  { 'i', UF_INVIS    },
  { 'o', UF_IRCOP    },
  { 'd', UF_DEAF     },
...
/* login to our uplink */
static unsigned int unreal33_server_login(void)
{
	int ret;
...
/* introduce a client */
static void unreal33_introduce_nick(user_t *u)
{
	const char *umode = user_get_umodestr(u);

	sts("NICK %s 1 %lu %s %s %s * %sS * :%s", u->nick, (unsigned long)u->ts, u->user, u->host, ME, umode, u->gecos);
}

/* invite a user to a channel */
static void unreal33_invite_sts(user_t *sender, user_t *target, channel_t *channel)
{
	sts(":%s INVITE %s %s", sender->nick, target->nick, channel->name);
}

static void unreal33_quit_sts(user_t *u, const char *reason)
{
	if (!me.connected)
		return;
...
/* WALLOPS wrapper */
static void unreal33_wallops_sts(const char *text)
{
	sts(":%s GLOBOPS :%s", ME, text);
}

/* join a channel */
static void unreal33_join_sts(channel_t *c, user_t *u, bool isnew, char *modes)
{
	if (isnew)
...
/* lower TS */
static void unreal33_chan_lowerts(channel_t *c, user_t *u)
{
	slog(LG_DEBUG, "unreal33_chan_lowerts(): lowering TS for %s to %lu",
...
static void unreal33_kick(user_t *source, channel_t *c, user_t *u, const char *reason)
{
...
/* PRIVMSG wrapper */
static void unreal33_msg(const char *from, const char *target, const char *fmt, ...)
{
	va_list ap;
	char buf[BUFSIZE];
...
static void unreal33_msg_global_sts(user_t *from, const char *mask, const char *text)
{
	mowgli_node_t *n;
	tld_t *tld;
...
/* NOTICE wrapper */
static void unreal33_notice_user_sts(user_t *from, user_t *target, const char *text)
{
	sts(":%s NOTICE %s :%s", from ? CLIENT_NAME(from) : ME, CLIENT_NAME(target), text);
}

static void unreal33_notice_global_sts(user_t *from, const char *mask, const char *text)
{
	mowgli_node_t *n;
	tld_t *tld;
...
static void unreal33_notice_channel_sts(user_t *from, channel_t *target, const char *text)
{
	sts(":%s NOTICE %s :%s", from ? CLIENT_NAME(from) : ME, target->name, text);
}

static void unreal33_numeric_sts(server_t *from, int numeric, user_t *target, const char *fmt, ...)
{
	va_list ap;
	char buf[BUFSIZE];
...
/* KILL wrapper */
static void unreal33_kill_id_sts(user_t *killer, const char *id, const char *reason)
{
	if (killer != NULL)
	{
...
/* PART wrapper */
static void unreal33_part_sts(channel_t *c, user_t *u)
{
	sts(":%s PART %s", u->nick, c->name);
}

/* server-to-server KLINE wrapper */
static void unreal33_kline_sts(const char *server, const char *user, const char *host, long duration, const char *reason)
{
	service_t *svs;
	...
	sts(":%s TKL + G %s %s %s %lu %lu :%s", ME, user, host, svs != NULL ? svs->nick : ME, (unsigned long)(duration > 0 ? CURRTIME + duration : 0), (unsigned long)CURRTIME, reason);
}

/* server-to-server UNKLINE wrapper */
static void unreal33_unkline_sts(const char *server, const char *user, const char *host)
{
	service_t *svs;
...
static void unreal33_xline_sts(const char *server, const char *realname, long duration, const char *reason)
{
	...
	sts(":%s BR + %s :%s", ME, escapedreason, realname);
}

static void unreal33_unxline_sts(const char *server, const char *realname)
{
	if (!me.connected)
		return;

	sts(":%s BR - :%s", ME, realname);
}

static void unreal33_qline_sts(const char *server, const char *name, long duration, const char *reason)
{
	service_t *svs;
...
static void unreal33_unqline_sts(const char *server, const char *name)
{
	service_t *svs;
...
/* topic wrapper */
static void unreal33_topic_sts(channel_t *c, user_t *source, const char *setter, time_t ts, time_t prevts, const char *topic)
{
	if (!me.connected || !c)
		return;
...
/* mode wrapper */
static void unreal33_mode_sts(char *sender, channel_t *target, char *modes)
{
	if (!me.connected)
		return;
...
/* ping wrapper */
static void unreal33_ping_sts(void)
{
	if (!me.connected)
		return;
...
static void unreal33_on_login(user_t *u, myuser_t *account, const char *wantedhost)
{
...
static bool unreal33_on_logout(user_t *u, const char *account)
{
	if (!me.connected || u == NULL)
		return false;
...
static void unreal33_jupe(const char *server, const char *reason)
{
	service_t *svs;
	...
	sts(":%s SERVER %s 2 :%s", ME, server, reason);
}

static void unreal33_sethost_sts(user_t *source, user_t *target, const char *host)
{
	if (!me.connected)
		return;
...
static void unreal33_fnc_sts(user_t *source, user_t *u, char *newnick, int type)
{
	sts(":%s SVSNICK %s %s %lu", source->nick, u->nick, newnick,
			(unsigned long)(CURRTIME - 60));
}

static void unreal33_holdnick_sts(user_t *source, int duration, const char *nick, myuser_t *mu)
{
	if (duration > 0)
...
static void unreal33_user_mode(user_t *u, const char *changes)
{
	const char *p;
	int dir;
...
static void m_mode(sourceinfo_t *si, int parc, char *parv[])
{
	if (*parv[0] == '#')
		...
		unreal33_user_mode(user_find(parv[0]), parv[1]);
}

static void m_umode(sourceinfo_t *si, int parc, char *parv[])
{
	unreal33_user_mode(si->su, parv[0]);
}
...
void _modinit(module_t * m)
{
	...
	quit_sts = &unreal33_quit_sts;
	wallops_sts = &unreal33_wallops_sts;
	join_sts = &unreal33_join_sts;
	kick = &unreal33_kick;
	msg = &unreal33_msg;
	msg_global_sts = &unreal33_msg_global_sts;
	notice_user_sts = &unreal33_notice_user_sts;
	notice_global_sts = &unreal33_notice_global_sts;
	notice_channel_sts = &unreal33_notice_channel_sts;
	numeric_sts = &unreal33_numeric_sts;
	kill_id_sts = &unreal33_kill_id_sts;
	part_sts = &unreal33_part_sts;
	kline_sts = &unreal33_kline_sts;
	unkline_sts = &unreal33_unkline_sts;
	xline_sts = &unreal33_xline_sts;
	unxline_sts = &unreal33_unxline_sts;
	qline_sts = &unreal33_qline_sts;
	unqline_sts = &unreal33_unqline_sts;
	topic_sts = &unreal33_topic_sts;
	mode_sts = &unreal33_mode_sts;
	ping_sts = &unreal33_ping_sts;
	ircd_on_login = &unreal33_on_login;
	ircd_on_logout = &unreal33_on_logout;
	jupe = &unreal33_jupe;
	sethost_sts = &unreal33_sethost_sts;
	fnc_sts = &unreal33_fnc_sts;
	invite_sts = &unreal33_invite_sts;
	holdnick_sts = &unreal33_holdnick_sts;
	chan_lowerts = &unreal33_chan_lowerts;

	mode_list = unreal33_mode_list;
	ignore_mode_list = unreal33_ignore_mode_list;
	status_mode_list = unreal33_status_mode_list;
	prefix_mode_list = unreal33_prefix_mode_list;
	user_mode_list = unreal33_user_mode_list;
	ignore_mode_list_size = ARRAY_SIZE(unreal33_ignore_mode_list);
