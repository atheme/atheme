 ...
 *
 * $Id: ultimate3.c 8301 2007-05-20 13:22:15Z jilles $
 */
...
DECLARE_MODULE_V1("protocol/ultimate3", true, _modinit, NULL, PACKAGE_STRING, "Atheme Development Group <http://www.atheme.org>");
...
struct cmode_ ultimate3_user_mode_list[] = {
  { 'A', UF_ADMIN    },
  { 'i', UF_INVIS    },
  { 'o', UF_IRCOP    },
  { 'd', UF_DEAF     },
...
static void ultimate3_introduce_nick(user_t *u)
{
	...
	sts("CLIENT %s 1 %lu %sS + %s %s * %s 0 0 :%s", u->nick, (unsigned long)u->ts, umode, u->user, u->host, me.name, u->gecos);
}
...
/* WALLOPS wrapper */
static void ultimate3_wallops_sts(const char *text)
{
	sts(":%s GLOBOPS :%s", chansvs.nick, text);
}
...
static void ultimate3_notice_global_sts(user_t *from, const char *mask, const char *text)
{
	node_t *n;
	tld_t *tld;
...
static void ultimate3_notice_channel_sts(user_t *from, channel_t *target, const char *text)
{
	sts(":%s NOTICE %s :%s", from ? from->nick : me.name, target->name, text);
}

static void ultimate3_numeric_sts(server_t *from, int numeric, user_t *target, const char *fmt, ...)
{
	va_list ap;
	char buf[BUFSIZE];
...
/* PART wrapper */
static void ultimate3_part_sts(channel_t *c, user_t *u)
{
	sts(":%s PART %s", u->nick, c->name);
}

/* server-to-server KLINE wrapper */
static void ultimate3_kline_sts(const char *server, const char *user, const char *host, long duration, const char *reason)
{
	if (!me.connected)
		return;
...
static void ultimate3_unkline_sts(const char *server, const char *user, const char *host)
{
	if (!me.connected)
		return;
...
/* topic wrapper */
static void ultimate3_topic_sts(channel_t *c, user_t *source, const char *setter, time_t ts, time_t prevts, const char *topic)
{
	if (!me.connected || !c)
		return;
...
/* mode wrapper */
static void ultimate3_mode_sts(char *sender, channel_t *target, char *modes)
{
	if (!me.connected)
		return;
...
static void ultimate3_sethost_sts(user_t *source, user_t *target, const char *host)
{
	if (!me.connected)
		return;
	...
	sts(":%s SVSMODE %s +x", me.name, target->nick);
	sts(":%s SETHOST %s :%s", me.name, target->nick, host);
}
...
void _modinit(module_t * m)
{
	...
	pcommand_add("CLIENT", m_nick, 2, MSRC_USER | MSRC_SERVER);
