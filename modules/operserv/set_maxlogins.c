...
static void
os_cmd_set_maxlogins_func(struct sourceinfo *const restrict si, const int parc, char **const restrict parv)
{
	if (! parc)
	{
...
mod_init(struct module *const restrict m)
{
	...
	(void) command_add(&os_cmd_set_maxlogins, *os_set_cmdtree);
}

static void
mod_deinit(const enum module_unload_intent ATHEME_VATTR_UNUSED intent)
{
	(void) command_delete(&os_cmd_set_maxlogins, *os_set_cmdtree);
}

SIMPLE_DECLARE_MODULE_V1("operserv/set_maxlogins", MODULE_UNLOAD_CAPABILITY_OK)
