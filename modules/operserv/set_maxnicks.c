...
static void
os_cmd_set_maxnicks_func(struct sourceinfo *const restrict si, const int parc, char **const restrict parv)
{
	if (! parc)
	{
...
static struct command os_cmd_set_maxnicks = {
	.name           = "MAXNICKS",
	.desc           = N_("Changes the maximum number of nicks that one account may own."),
	.access         = PRIV_ADMIN,
	.maxparc        = 1,
	.cmd            = &os_cmd_set_maxnicks_func,
...
mod_init(struct module *const restrict m)
{
	...
	(void) command_add(&os_cmd_set_maxnicks, *os_set_cmdtree);
}

static void
mod_deinit(const enum module_unload_intent ATHEME_VATTR_UNUSED intent)
{
	(void) command_delete(&os_cmd_set_maxnicks, *os_set_cmdtree);
}

SIMPLE_DECLARE_MODULE_V1("operserv/set_maxnicks", MODULE_UNLOAD_CAPABILITY_OK)
