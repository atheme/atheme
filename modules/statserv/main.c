...
command_t ss_help = { "HELP", N_(N_("Displays contextual help information.")), AC_NONE, 2, ss_cmd_help, { .path = "help" } };
...
void ss_cmd_help(sourceinfo_t *si, int parc, char *parv[])
{
    ...
    if (!command)
    {
		...
		command_success_nodata(si, _("\2%s\2 records various network statistics.."), si->service->nick);
