 ...
 *
 * $Id: setpass.c 7801 2007-03-04 21:53:13Z jilles $
 */
...
static void ns_cmd_setpass(sourceinfo_t *si, int parc, char *parv[])
{
	...
	if (md != NULL && crypt_verify_password(key, md->value))
	{
		...
		command_success_nodata(si, "The password for \2%s\2 has been changed to \2%s\2. Please write this down for future reference.", mu->name, password);
