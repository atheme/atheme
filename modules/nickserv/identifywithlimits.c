 ...
 * 
 * Similar to the nickserv/identify command, except with the ability
 * to stop brute-force style attacks by akilling failing users after a
 * configured amount of attempts for a configurable amount of seconds.
 *
 * Configuration of this module is done via the below two lines
 * (identify_failures_*).  AKILL time is by default 3600 seconds
 * or one hour.  Default FAIL count is 10 before AKILLs.
 *
 */

#include "atheme.h"

static size_t identify_failures_allowed = 10;
static time_t identify_failures_akilltime = 3600;
...
#define COMMAND_LC "identifywithlimits"
#endif
...
#else
command_t ns_identify = { "IDENTIFY", N_("Identifies to services for a nickname."), AC_NONE, 2, ns_cmd_login, { .path = "nickserv/identifywithlimits" } };
#endif
...
static void ns_cmd_login(sourceinfo_t *si, int parc, char *parv[])
{
	...
	if (count >= identify_failures_allowed) 
	{
	k = kline_add("*", si->su->host, "Too many failed identification attempts.", identify_failures_akilltime, "Services");
	}
