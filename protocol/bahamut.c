 ...
 *
 * $Id: bahamut.c 1997 2005-09-01 04:28:13Z nenolod $
 */

#include "atheme.h"
#include "protocol/bahamut.h"

DECLARE_MODULE_V1("protocol/bahamut", FALSE, _modinit, NULL);
...
/* introduce a client */
static user_t *bahamut_introduce_nick(char *nick, char *user, char *host, char *real, char *modes)
{
	user_t *u;
...
static void m_privmsg(char *origin, uint8_t parc, char *parv[])
{
	...
	if ((config_options.flood_msgs) && (!is_sra(u->myuser)) && (!is_ircop(u)) && parv[0][0] != '#')
	{
...
static void m_nick(char *origin, uint8_t parc, char *parv[])
{
	...
	if (parc == 10)
	{
		...
		u = user_add(parv[0], parv[4], parv[5], NULL, NULL, parv[9], s);
