 ...
 *
 * $Id: hybrid.c 6291 2006-09-06 02:26:55Z pippijn $
 */
...
DECLARE_MODULE_V1("protocol/hybrid", TRUE, _modinit, NULL, "$Id: hybrid.c 6291 2006-09-06 02:26:55Z pippijn $", "Atheme Development Group <http://www.atheme.org>");
...
/* NOTICE wrapper */
static void hybrid_notice(char *from, char *target, char *fmt, ...)
{
	va_list ap;
	char buf[BUFSIZE];
	user_t *u = user_find(from);
	...
	{
		slog(LG_DEBUG, "hybrid_notice(): unknown source %s for notice to %s", from, target);
		return;
	}
...
/* topic wrapper */
static void hybrid_topic_sts(char *channel, char *setter, time_t ts, char *topic)
{
	channel_t *c;
	int joined = 0;
...
static void m_tb(sourceinfo_t *si, uint8_t parc, char *parv[])
{
	channel_t *c = channel_find(parv[0]);
...
static void m_bmask(sourceinfo_t *si, uint8_t parc, char *parv[])
{
	uint8_t ac, i;
	char *av[256];
...
static void m_uid(sourceinfo_t *si, uint8_t parc, char *parv[])
{
	server_t *s;
	user_t *u;
...
static void m_tmode(sourceinfo_t *si, uint8_t parc, char *parv[])
{
	channel_t *c;
...
static void m_sid(sourceinfo_t *si, uint8_t parc, char *parv[])
{
