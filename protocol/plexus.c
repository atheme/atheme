 ...
 *
 * $Id: plexus.c 2491 2005-10-01 04:26:53Z nenolod $
 */

#include "atheme.h"
#include "protocol/plexus.h"

DECLARE_MODULE_V1("protocol/plexus", TRUE, _modinit, NULL, "$Id: plexus.c 2491 2005-10-01 04:26:53Z nenolod $", "Atheme Development Group <http://www.atheme.org>");
...
/* introduce a client */
static user_t *plexus_introduce_nick(char *nick, char *user, char *host, char *real, char *modes)
{
	user_t *u;

	sts("NICK %s 1 %ld +%sS %s %s %s %s 0 0 :%s", nick, CURRTIME, modes, user, host, host, me.name, real);
...
/* join a channel */
static void plexus_join(char *chan, char *nick)
{
...
/* topic wrapper */
static void plexus_topic_sts(char *channel, char *setter, char *topic)
{
	if (!me.connected)
		return;
...
static void m_nick(char *origin, uint8_t parc, char *parv[])
{
	...
	if (parc == 10)
	{
		...
		u = user_add(parv[0], parv[4], parv[5], parv[6], NULL, NULL, parv[9], s);
