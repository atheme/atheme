 ...
 *
 * $Id: ircnet.c 6291 2006-09-06 02:26:55Z pippijn $
 */
...
DECLARE_MODULE_V1("protocol/ircnet", TRUE, _modinit, NULL, "$Id: ircnet.c 6291 2006-09-06 02:26:55Z pippijn $", "Atheme Development Group <http://www.atheme.org>");
...
static void m_eob(sourceinfo_t *si, uint8_t parc, char *parv[])
{
	server_t *serv;
	char sidbuf[4+1], *p;
...
static void m_njoin(sourceinfo_t *si, uint8_t parc, char *parv[])
{
	channel_t *c;
	uint8_t userc;
	char *userv[256];
...
static void m_server(sourceinfo_t *si, uint8_t parc, char *parv[])
{
	...
	server_add(parv[0], atoi(parv[1]), si->origin ? si->origin : me.name, parv[2], parv[parc - 1]);
